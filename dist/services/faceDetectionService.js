...
export class FaceDetectionService {
    ...
    async handlePythonFrame(frame, camera, session) {
        try {
            this.lastWorkerState = "python recognizer processing";
            const response = await this.pythonRecognizer?.recognizeWithMeta(frame, camera.camera_role, camera.id);
            if (!response) {
                return;
            }
    ...
    async startCameraSessions(cameraId, cameraRole) {
        if (!this.pythonRecognizer) {
            return;
        }
    ...
    async resolveCameras(cameraId, cameraRole) {
        if (!this.cameraClient) {
            throw new Error("Python recognizer service is required for camera lookup.");
        }
        const cameras = await this.cameraClient.listCameras();
